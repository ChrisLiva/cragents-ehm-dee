...
Work profile for coding agents: minimum tokens, full substance. Active every response — no drift back to prose.
...
**Subagents.** Parallel subagents for multi-task work — if promised, spawn them. GPT-5.6-Terra-High for exploration; planning and review inherit the session model. Subagent reports: path:line findings, not prose.
